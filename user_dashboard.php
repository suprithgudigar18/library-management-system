...
foreach ($myRequests as &$req) {
    if ($req['status']==='Approved' && $req['due_date'] && !$req['returned_at'] && !($req['fine_paid']??0)) {
        ...
        if ($today>$due) {
            $fine=(int)$today->diff($due)->days*10;
...
.book-card:hover{transform:translateY(-6px);
    box-shadow:0 20px 50px rgba(0,0,0,.65),0 0 0 1px rgba(88,166,255,.2);
    border-color:rgba(88,166,255,.4);}

/* Cover */
.bc-img{height:190px;overflow:hidden;position:relative;background:#161b22;flex-shrink:0;}
.bc-img img{width:100%;height:100%;object-fit:cover;transition:transform .4s ease;}
.book-card:hover .bc-img img{transform:scale(1.07);}
.bc-placeholder{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;}
.bc-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.75) 0%,transparent 55%);opacity:0;transition:opacity .25s;}
.book-card:hover .bc-overlay{opacity:1;}

/* Copies pill */
.bc-copies{position:absolute;top:8px;right:8px;border-radius:20px;padding:3px 10px;
    font-size:.68rem;font-weight:700;backdrop-filter:blur(6px);
    display:flex;align-items:center;gap:4px;background:rgba(0,0,0,.75);}
...
/* Body */
.bc-body{padding:13px 14px 9px;flex:1;display:flex;flex-direction:column;gap:5px;}
.bc-title{font-weight:700;color:white;font-size:.9rem;line-height:1.35;
...
/* Description: emphasised, always visible */
.bc-desc{font-size:.75rem;color:rgba(255,255,255,.5);line-height:1.55;margin-top:4px;
    display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;flex:1;}
.bc-desc.has-desc{color:rgba(255,255,255,.55);}
.bc-desc.no-desc{font-style:italic;color:rgba(255,255,255,.25);-webkit-line-clamp:1;}

/* ISBN chip */
.bc-isbn{font-size:.62rem;font-family:monospace;color:rgba(255,255,255,.2);margin-top:2px;}

/* Footer */
.bc-footer{padding:0 14px 13px;}
.bc-btn{width:100%;padding:9px 8px;border-radius:10px;font-weight:700;font-size:.8rem;
    cursor:pointer;border:none;transition:all .2s;
    display:flex;align-items:center;justify-content:center;gap:6px;}
.bc-btn.can{background:linear-gradient(135deg,#238636,#1a7f37);color:white;
    box-shadow:0 4px 14px rgba(35,134,54,.3);}
.bc-btn.can:hover{background:linear-gradient(135deg,#2ea043,#238636);
    box-shadow:0 6px 22px rgba(35,134,54,.45);transform:translateY(-1px);}
...
/* AI FAB */
#ai-fab{position:fixed;bottom:28px;right:28px;z-index:8000;width:58px;height:58px;
    border-radius:50%;background:linear-gradient(135deg,#238636,#1a7f37);
    border:2px solid rgba(63,185,80,.4);box-shadow:0 8px 30px rgba(35,134,54,.45);
    cursor:pointer;display:flex;align-items:center;justify-content:center;
    font-size:1.5rem;transition:transform .25s,box-shadow .25s;
    text-decoration:none;animation:fabPulse 2.5s ease-in-out infinite;}
...
#ai-fab-label{position:absolute;bottom:68px;right:8px;background:var(--card);border:1px solid var(--border);
    color:white;font-size:.72rem;font-weight:600;padding:4px 10px;border-radius:8px;white-space:nowrap;
    box-shadow:0 4px 12px rgba(0,0,0,.4);pointer-events:none;opacity:0;transition:opacity .2s;}
...
<?php // Toast triggers
$toastMap=['photo_ok'=>['✅ Photo updated!','#238636'],'profile_ok'=>['✅ Profile saved!','#238636'],
           'photo_err'=>['❌ Use JPG/PNG under 2MB','#f85149'],'purchase_ok'=>['✅ Request sent to admin!','#238636'],
...
<div id="apprOv" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background:rgba(0,0,0,.85);backdrop-filter:blur(6px)">
  <div class="card max-w-md w-full p-6 text-center">
    ...
    <?php endforeach; ?>
    <p class="text-xs mt-3 mb-5" style="color:var(--muted)">Collect from library counter. Return within 4 days.</p>
...
<div id="pane-browse">
    ...
    <div class="book-card"
        ...
        <div class="bc-body">
            ...
            <div class="bc-tags">
                <?php if(!empty($b['category'])):?><span class="bc-tag bc-tag-cat"><?=htmlspecialchars($b['category'])?></span><?php endif;?>
                <?php if(!empty($b['genre']) && $b['genre']!==$b['category']):?><span class="bc-tag bc-tag-genre"><?=htmlspecialchars($b['genre'])?></span><?php endif;?>
                <?php if(!empty($b['shelf'])):?><span class="bc-tag bc-tag-shelf">📍 <?=htmlspecialchars($b['shelf'])?></span><?php endif;?>
            </div>
            <div class="bc-desc <?=$desc?'has-desc':'no-desc'?>"><?=$desc?htmlspecialchars($desc):'No description available'?></div>
            <?php if(!empty($b['isbn'])):?><div class="bc-isbn">ISBN: <?=htmlspecialchars($b['isbn'])?></div><?php endif;?>
        </div>

        <div class="bc-footer">
            <?php if($alreadyReq):?>
                <button class="bc-btn done" disabled>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg> Already Requested
                </button>
            <?php else:?>
                <form method="POST">
                    <input type="hidden" name="request_book_id" value="<?=$b['id']?>">
                    <button type="submit" class="bc-btn <?=$canReq?'can':'none'?>" <?=!$canReq?'disabled':''?>>
                        <?php if($canReq):?>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg> Request Book
                        <?php else:?>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg> Unavailable
                        <?php endif;?>
                    </button>
                </form>
            <?php endif;?>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr>
                    ...
                    <td class="text-sm <?=$ov?'text-red-400 font-bold':''?>" style="<?=!$ov?'color:var(--muted)':''?>"><?=$req['due_date']?date('d M Y',strtotime($req['due_date'])):'—'?><?php if($ov):?> ⚠️<?php endif;?></td>
...
<div id="pane-fines" class="hidden">
    ...
    <div class="grid md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="card overflow-hidden md:col-span-2">
            ...
            <div class="px-5 py-3 text-xs" style="color:var(--muted);border-top:1px solid var(--border)">💡 Submit screenshot after paying. Admin will verify and clear your fine.</div>
        </div>
    </div>
    <?php else:?>
