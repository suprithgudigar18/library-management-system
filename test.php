...
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS `fine_payments` (`id` INT AUTO_INCREMENT PRIMARY KEY,`user_id` INT NOT NULL,`request_id` INT NOT NULL,`amount` DECIMAL(10,2) NOT NULL,`upi_ref` VARCHAR(100) DEFAULT '',`screenshot` VARCHAR(255) DEFAULT '',`status` ENUM('Pending','Verified','Rejected') DEFAULT 'Pending',`submitted_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,`verified_at` DATETIME DEFAULT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $pdo->exec("CREATE TABLE IF NOT EXISTS `book_purchase_requests` (`id` INT AUTO_INCREMENT PRIMARY KEY,`user_id` INT NOT NULL,`book_title` VARCHAR(255) NOT NULL,`author` VARCHAR(150) DEFAULT '',`reason` TEXT,`status` ENUM('Pending','Reviewed','Ordered','Rejected') DEFAULT 'Pending',`admin_note` TEXT,`created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    try { $pdo->exec("ALTER TABLE `book_requests` ADD COLUMN `fine_paid` TINYINT(1) DEFAULT 0"); } catch(Exception $e){}
} catch (Exception $e) {}

// CORRECT FINE CALCULATION: Rs10 per day from day 1 of being overdue
// ✅ CUSTOM FINE RULE: 4 days = ₹20, after that ₹10/day
$today = new DateTime();

// Approved (not returned)
$allApproved = $pdo->query("
    SELECT id, due_date 
    FROM book_requests 
...
foreach ($allApproved as $row) {
    ...
    if ($today > $due) {
        ...
        $pdo->prepare("
            ...
            SET fine_amount=? 
            WHERE id=? AND (fine_paid IS NULL OR fine_paid=0)
        ")->execute([$fine, $row['id']]);

    } else {
        $pdo->prepare("UPDATE book_requests SET fine_amount=0 WHERE id=?")
            ->execute([$row['id']]);
    }
}


// Returned books (IMPORTANT)
$allReturned = $pdo->query("
...
foreach ($allReturned as $row) {
    $due = new DateTime($row['due_date']);
    $returned = new DateTime($row['returned_at']);

    if ($returned > $due) {
        $days = (int)$due->diff($returned)->days;
...
// Fetch
try {
    $finePayments = $pdo->query("SELECT fp.*, u.username, u.full_name, b.title AS book_title, br.fine_amount, br.due_date FROM fine_payments fp JOIN users u ON u.id=fp.user_id JOIN book_requests br ON br.id=fp.request_id JOIN books b ON b.id=br.book_id ORDER BY FIELD(fp.status,'Pending','Rejected','Verified'), fp.submitted_at DESC")->fetchAll();
} catch (Exception $e) { $finePayments = []; }
try {
    $purchaseRequests = $pdo->query("SELECT bpr.*, u.username, u.full_name FROM book_purchase_requests bpr JOIN users u ON u.id=bpr.user_id ORDER BY FIELD(bpr.status,'Pending','Reviewed','Ordered','Rejected'), bpr.created_at DESC")->fetchAll();
...
:root{--accent:#22d3ee;--onyx:#02040a;--glass:rgba(255,255,255,.03);--border:rgba(255,255,255,.1);--red:#f87171;--green:#34d399;--yellow:#fbbf24;--blue:#60a5fa;--muted:rgba(255,255,255,.5);--purple:#a78bfa;}
*{margin:0;padding:0;box-sizing:border-box;}
body{background:var(--onyx);font-family:'Inter',sans-serif;color:white;display:flex;min-height:100vh;}
.sidebar{position:fixed;top:0;left:0;height:100vh;width:250px;background:rgba(0,0,0,.4);padding:2rem 1rem;border-right:1px solid var(--border);z-index:1000;box-shadow:2px 0 15px rgba(0,0,0,.3);overflow-y:auto;}
...
.nav-item{display:flex;align-items:center;padding:.8rem 1.2rem;margin:.3rem 0;text-decoration:none;color:rgba(255,255,255,.8);border-radius:8px;transition:all .3s;white-space:nowrap;gap:10px;font-size:.9rem;}
...
.page-icon{width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.4rem;background:rgba(34,211,238,.1);border:1px solid rgba(34,211,238,.2);}
.page-title{font-family:'Playfair Display',serif;font-size:1.8rem;}
.page-sub{font-size:.82rem;color:var(--muted);margin-top:2px;}
.back-btn{display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;background:var(--glass);border:1px solid var(--border);color:rgba(255,255,255,.6);text-decoration:none;font-size:.82rem;transition:all .25s;}
.back-btn:hover{border-color:var(--accent);color:var(--accent);}
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(155px,1fr));gap:1.2rem;margin-bottom:2rem;}
.stat-card{background:var(--glass);backdrop-filter:blur(10px);border:1px solid var(--border);border-radius:16px;padding:1.3rem;text-align:center;}
...
.fine-rule-box{background:rgba(251,191,36,.06);border:1px solid rgba(251,191,36,.2);border-radius:10px;padding:10px 16px;font-size:.8rem;color:rgba(255,255,255,.55);margin-bottom:1.5rem;}
...
.filter-pill{padding:4px 14px;border-radius:20px;border:1px solid var(--border);background:var(--glass);color:var(--muted);font-size:.76rem;font-weight:600;cursor:pointer;transition:all .2s;}
...
.box-admin{background:rgba(34,211,238,.07);border:1px solid rgba(34,211,238,.2);color:rgba(255,255,255,.6);}
...
.btn{display:inline-flex;align-items:center;gap:6px;padding:8px 18px;border-radius:9px;font-size:.82rem;font-weight:700;cursor:pointer;border:none;transition:all .25s;}
...
.btn-send{background:rgba(34,211,238,.1);border:1px solid var(--accent);color:var(--accent);padding:9px 20px;border-radius:9px;font-size:.85rem;font-weight:700;cursor:pointer;transition:all .25s;white-space:nowrap;}
...
.gi{width:100%;padding:9px 13px;background:rgba(255,255,255,.06);border:1px solid var(--border);border-radius:9px;color:white;font-family:'Inter',sans-serif;font-size:.85rem;outline:none;transition:border-color .25s;}
...
.empty{text-align:center;padding:4rem 2rem;color:var(--muted);background:var(--glass);border:1px solid var(--border);border-radius:16px;}
.empty i{font-size:2.5rem;margin-bottom:1rem;display:block;opacity:.3;}
.empty h3{color:white;margin-bottom:6px;}
.toast-box{position:fixed;bottom:24px;right:24px;z-index:9999;padding:12px 20px;border-radius:12px;font-weight:700;font-size:.88rem;display:flex;align-items:center;gap:8px;animation:ti .4s ease,to .5s 3.5s forwards;}
...
<aside class="sidebar">
    ...
    <a href="book_requests.php"         class="nav-item"><i class="fas fa-clipboard-list"></i> Book Requests<?php if($pendingReqs>0):?><span class="nav-badge"><?=$pendingReqs?></span><?php endif;?></a>
    <a href="admin_requests_extra.php"  class="nav-item active"><i class="fas fa-credit-card"></i> Payments & Orders<?php if($pendingPay+$pendingPurchase>0):?><span class="nav-badge"><?=$pendingPay+$pendingPurchase?></span><?php endif;?></a>
...
<main class="main">
    ...
    <div class="fine-rule-box">
       ...
       <strong>Fine Rule:</strong> ₹20 for first 4 days, then ₹10 per extra day — starts from day 1 after the 4-day loan period.
        Example: 5 days overdue = ₹50. Fines auto-update on every page load.
    </div>
    ...
    <div id="pane-pay">
        ...
        <div class="empty"><i class="fas fa-receipt"></i><h3>No payment submissions yet</h3><p>Students submit payment proofs from their Fine Management tab — they appear here.</p></div>
        ...
        <div class="req-card <?=$bl?> pay-card" data-status="<?=$fp['status']?>">
            <div class="card-top">
                <div style="flex:1">
                    ...
                    <div class="card-meta">
                        <span class="chip chip-user"><i class="fas fa-user" style="font-size:.6rem"></i><?=htmlspecialchars($fp['full_name']?:$fp['username'])?></span>
                        <span class="chip chip-amt">Paid: ₹<?=number_format($fp['amount'],2)?></span>
                        <?php if($daysLate>0):?><span class="chip chip-days"><i class="fas fa-calendar-times" style="font-size:.6rem"></i><?=$daysLate?> days late — Should be ₹<?=$calcFine?></span><?php endif;?>
            ...
            <?php if($fp['upi_ref']):?><div class="info-box box-upi" style="margin-top:10px"><i class="fas fa-fingerprint" style="margin-right:6px"></i>UPI Ref: <strong><?=htmlspecialchars($fp['upi_ref'])?></strong></div><?php endif;?>
            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:10px">
                <?php if($fp['screenshot']):?><a href="<?=htmlspecialchars($fp['screenshot'])?>" target="_blank" class="screenshot-link"><i class="fas fa-image"></i> View Screenshot</a><?php else:?><span style="font-size:.75rem;color:var(--muted)"><i class="fas fa-image" style="margin-right:4px;opacity:.4"></i>No screenshot</span><?php endif;?>
            ...
            <div class="action-row">
                <form method="POST" style="display:inline" onsubmit="return confirm('Verify and clear this fine?')">
                ...
                <form method="POST" style="display:inline" onsubmit="return confirm('Reject this payment?')">
                    ...
                    <input type="hidden" name="pay_id" value="<?=$fp['id']?>">
                    <button type="submit" class="btn btn-r"><i class="fas fa-times-circle"></i> Reject</button>
                </form>
            </div>
            <?php elseif($fp['status']==='Verified'):?><div style="margin-top:10px;font-size:.82rem;color:var(--green)"><i class="fas fa-check-circle"></i> Verified <?=$fp['verified_at']?date('d M Y',strtotime($fp['verified_at'])):'N/A'?> — fine cleared</div>
            <?php else:?><div style="margin-top:10px;font-size:.82rem;color:var(--red)"><i class="fas fa-times-circle"></i> Payment rejected</div><?php endif;?>
        </div>
    ...
    <div id="pane-order" style="display:none">
        ...
        <div class="empty"><i class="fas fa-box-open"></i><h3>No book order requests yet</h3><p>Students submit requests from the "Request a Book" tab in their dashboard.</p></div>
        ...
        <div class="req-card <?=$bl?> order-card" data-status="<?=$pr['status']?>">
            <div class="card-top">
                <div style="flex:1">
                    ...
                    <div class="card-meta">
                        <span class="chip chip-user"><i class="fas fa-user" style="font-size:.6rem"></i><?=htmlspecialchars($pr['full_name']?:$pr['username'])?></span>
                ...
                <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
                    ...
                    <button class="btn btn-resp" onclick="tr('order-<?=$pr['id']?>')"><i class="fas fa-reply"></i> Respond</button>
                </div>
            </div>
            <?php if($pr['reason']):?><div class="info-box box-reason"><i class="fas fa-quote-left" style="opacity:.3;margin-right:6px;font-size:.75rem"></i><?=htmlspecialchars($pr['reason'])?></div><?php endif;?>
            <?php if($pr['admin_note']):?>
            <div class="info-box box-admin">
                <strong><i class="fas fa-reply" style="margin-right:4px"></i>Your response (shown to student):</strong><br>
                <span style="margin-top:4px;display:block"><?=htmlspecialchars($pr['admin_note'])?></span>
            </div>
            <?php endif;?>
            <div class="resp-form" id="order-<?=$pr['id']?>">
                <p style="font-size:.78rem;color:var(--muted);margin-bottom:10px"><i class="fas fa-info-circle" style="margin-right:4px"></i>Your response will be shown to the student instantly in their <strong style="color:white">Request a Book</strong> tab.</p>
                <form method="POST">
                    ...
                    <div class="fg">
                        ...
                        <div>
                            ...
                            <input type="text" name="admin_note" class="gi" placeholder="e.g. Book ordered! Arriving next week. / Not in budget this semester." value="<?=htmlspecialchars($pr['admin_note']??'')?>">
                        </div>
                        <div><label class="lbl">&nbsp;</label><button type="submit" class="btn-send"><i class="fas fa-paper-plane"></i> Send</button></div>
                    </div>
                    <div class="status-legend">
                        <span><span style="color:var(--yellow)">⏳ Pending</span> = not reviewed yet</span>
                        <span><span style="color:var(--blue)">👀 Reviewed</span> = you've acknowledged it</span>
                        <span><span style="color:var(--green)">✅ Ordered</span> = book is being ordered</span>
                        <span><span style="color:var(--red)">❌ Rejected</span> = cannot fulfil request</span>
                    </div>
                </form>
            </div>
        </div>
...
function fc(pane,status,btn){const cls=pane==='pay'?'.pay-card':'.order-card';document.querySelectorAll('#pane-'+pane+' '+cls).forEach(c=>{c.style.display=(status==='All'||c.dataset.status===status)?'block':'none';});btn.closest('.filter-row').querySelectorAll('.filter-pill').forEach(p=>p.classList.remove('active'));btn.classList.add('active');}
